...
Today we complete the mathematical core of this course.  Three tasks:
 ...
 2. Show that sequential compactness is *equivalent to* "closed and bounded"
    ...
    theorem.
 3. Prove the **Cantor intersection theorem**:
    a nested sequence of nonempty sequentially compact subsets of `ℝ`
    has a nonempty intersection.
    This is the course's closing statement — it synthesizes
...
example (L M : ℝ): 2 *(|L-M|/2) =|L-M| := by rw [mul_div_cancel₀];  apply two_ne_zero
#print two_ne_zero
...
private theorem limit_unique (a : ℕ → ℝ) (L M : ℝ)
  ...
  obtain ⟨N₂, hN₂⟩ := hM _ hε
  let N := max N₁ N₂
  have h1 := hN₁ N (le_max_left _ _ : N₁ ≤ N)
  have h2 := hN₂ N (le_max_right _ _ : N₂ ≤ N)
  have : 2*ε < 2*ε := by
   calc
     2 * ε = |L-M| := by rw [εdef,mul_div_cancel₀]; apply two_ne_zero -- why this doesn't work? rw [εdef,mul_div_cancel₀ two_ne_zero]
     _     = |L - a N + a N - M| := by norm_num
     _     = |L - a N + (a N - M)| := by norm_num
     _     ≤ |L - a N| + |a N - M| := abs_add _ _
     _     < 2 * ε := by nth_rw 1 [abs_sub_comm]; linarith
  linarith
...
theorem evt_min {K : Set ℝ} (hK : IsSeqCompact K) (hKne : K.Nonempty)
  ...
  have hneg : ∀ c ∈ K, ContinuousAt (fun x => -f x) c := by
    ...
    use δ, hδ
    simp
    simp_rw [add_comm,←sub_eq_add_neg,abs_sub_comm (f c)] -- NOTE: simp_rw allows to sneak passed fun
    exact hδprop
...
theorem seqCompact_bounded {K : Set ℝ} (hK : IsSeqCompact K) :
  ...
  -- Seq. compactness implies a convergent subseq `a_{sₖ}`
  obtain ⟨s, w, hw, hsmono, hconv⟩ := hK x hxK
  -- This subsequence is bounded by some `M`, since it is convergent
  obtain ⟨M, hM⟩ := bounded_of_convergesTo hconv
  obtain ⟨N, hN⟩ := exists_nat_gt M
  -- but, it must have terms, e.g. `a_{s_N} > N > M` that violate that bound. Contradiction
...
-- ============================================================================
-- ## Part 5: Course finale — Cantor intersection theorem (CIT)
-- ============================================================================
...
theorem cantor_intersection (K : ℕ → Set ℝ)
    ...
    (hdesc : ∀ n, K (n + 1) ⊆ K n) :
    (⋂ n, K n).Nonempty := by
  -- Choose a sequence with `xₙ ∈ Kₙ`
  choose x hx using hne
  -- `hdesc` iterated by induction on `m ≤ n`: `K n ⊆ K m`.
  have hdesc_iter : ∀ m n, m ≤ n → K n ⊆ K m := by
    intro m n hmn
    induction' hmn with k ihk hkm
    . exact fun _ h => h
    . apply subset_trans (hdesc k) hkm
  -- Each `x n` lies in `K 0`, so seq-compactness of `K 0` applies.
  have hx0 : ∀ n, x n ∈ K 0 := by intro n; apply hdesc_iter 0 n (Nat.zero_le n) (hx n)
