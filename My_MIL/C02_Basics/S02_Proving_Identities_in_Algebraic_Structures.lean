...
theorem add_left_cancel {a b c : R} (h : a + b = a + c) : b = c := by
  calc
   b = -a + (a + b) := by rw [←add_assoc,neg_add_cancel, zero_add]
   _ = -a + (a + c) := by rw [h]
...
theorem mul_inv_cancel (a : G) : a * a⁻¹ = 1 := by
  calc
   a * a⁻¹ = (a * a⁻¹)⁻¹ * (a*a⁻¹) * (a*a⁻¹) := by rw [inv_mul_cancel,one_mul]
   _ = (a * a⁻¹)⁻¹ * (a*a⁻¹) := by rw [mul_assoc,mul_assoc a,← mul_assoc a⁻¹,inv_mul_cancel,one_mul]
   _ = 1 := by apply inv_mul_cancel
...
theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  ...
  nth_rw 2 [← mul_one a]
  rw [← mul_inv_cancel b,← mul_assoc a,mul_assoc,← mul_assoc,inv_mul_cancel,one_mul]
